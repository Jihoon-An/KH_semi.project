	...
	<div class="window">
		...
		<form class="search" id="form_search">
			<div class="row justify-content-center mt-4">
				<div class="col-12 gy-3">
					<input type="text" name="phone" id="search_phone"
...
<script>
			...
			function showSearch() {
				...
				$("#form_search").attr("style", "display:inline");
				$("#loginModal .window").attr("style", "height:360px");
			...
			function trySearch() {
				...
				$.get($("#loginModal .title").text() =="아이디 찾기" ? "/searchId.user" : "/searchPw.user", $("#form_search").serialize())
					.done((res => {
						if (res == "true") {
							// Fake Alert
			...
			$("#btn_search").on("click", () => {
				if (isFilled($("#search_phone"))) { trySearch(); }
			});

			// 엔터 = 버튼 클릭
			$("#login_id, #login_pw").on("keyup", (e) => { if (e.keyCode == 13) { $("#btn_login").click() } });
			$("#search_phone").on("keyup", (e) => { if (e.keyCode == 13) { $("#btn_search").click() } });;
		</script>